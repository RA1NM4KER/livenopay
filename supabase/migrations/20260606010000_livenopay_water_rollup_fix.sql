...
begin
  ...
  hourly_aggregates as (
    ...
    from public.energy_rows
    where charge_kind in ('energy', 'water')
  ...
  set
    ...
    max_water_interval_spend = coalesce((select max(water_spend) from public.energy_interval_rollups), 0),
    max_water_interval_kl = coalesce((select max(water_kl) from public.energy_interval_rollups), 0),
    updated_at = now(),
