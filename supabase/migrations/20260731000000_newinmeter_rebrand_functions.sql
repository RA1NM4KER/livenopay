-- Product renamed livenopay -> NewInMeter. This renames the internal
-- Postgres functions/trigger that carried the old name so the DB matches
-- the app codebase. Table/column names are untouched (none used the old
-- name), and "livemopay" identifiers are untouched -- that's the real
-- third-party service NewInMeter connects to, unrelated to our product name.

create or replace function public.parse_newinmeter_period_ts(value text)
...
create or replace function public.parse_newinmeter_capture_ts(value text)
returns timestamp without time zone
language sql
immutable
as $$
...
create or replace function public.handle_newinmeter_capture_run_refresh()
returns trigger
language plpgsql
as $$
begin
  if new.status = 'success' and new.finished_at is not null then
    perform public.refresh_newinmeter_rollups_for_run(new.id);
  end if;
...
drop trigger if exists capture_runs_refresh_rollups_insert on public.capture_runs;
...
drop trigger if exists capture_runs_refresh_rollups_update on public.capture_runs;
...
drop function if exists public.refresh_livenopay_rollups_for_run(uuid);
drop function if exists public.refresh_livenopay_rollups(uuid);
drop function if exists public.parse_livenopay_period_ts(text);
drop function if exists public.parse_livenopay_capture_ts(text);
