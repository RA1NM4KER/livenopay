-- This migration, the dashboard-data.ts read path, and the sync route /
-- livenopay-sync.ts rewrite are one deployable unit. The old `id = 1` global
-- summary row is replaced by one row per connection in the same migration
-- that rewrites the rollup function to write it and the application code
-- that reads it -- there is no intermediate state where deployed code
-- queries `id = 1` against a dashboard_summary that no longer has that
-- column.

-- Dead code: confirmed zero references anywhere in src/. dashboard-data.ts
-- has always queried the base table directly, never this view.
drop view if exists public.dashboard_summary_live;

drop policy if exists "dashboard summary is readable" on public.dashboard_summary;

-- connection_id must already be backfilled by scripts/backfill-legacy-owner.ts
-- before this runs -- `alter column ... set not null` fails loudly otherwise.
alter table public.dashboard_summary drop constraint dashboard_summary_pkey;
alter table public.dashboard_summary drop column id;
alter table public.dashboard_summary alter column connection_id set not null;
alter table public.dashboard_summary add primary key (connection_id);
...
-- Full rewrite: every delete/aggregate/insert is scoped to
-- run_record.connection_id in addition to the existing sync_run_id-based
-- "which dates changed" detection, and the summary write becomes an upsert
-- keyed on connection_id instead of an `update ... where id = 1`, so a
-- connection's first-ever sync creates its summary row instead of depending
-- on a pre-seeded row existing.
