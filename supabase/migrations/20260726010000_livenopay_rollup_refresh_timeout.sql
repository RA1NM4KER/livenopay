-- A --full sync on a large, long-lived account (19+ months of history)
-- recomputes day/hourly/interval rollups across every affected date in one
-- transaction. That legitimately takes longer than the role's default
-- statement_timeout, and the whole PATCH to capture_runs (which fires this
-- function via trigger) gets cancelled -- ledger data in energy_rows is
-- fine, but dashboard_summary/rollups are left stale.
--
-- set local scopes the raised timeout to just this function's transaction,
-- not the role generally, so it doesn't loosen protection against a runaway
-- query anywhere else.
